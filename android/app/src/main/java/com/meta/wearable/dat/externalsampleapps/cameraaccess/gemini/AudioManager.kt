...
class AudioManager {
    ...
    fun startCapture() {
        ...
        val bufferSize = AudioRecord.getMinBufferSize(
            HiveConfig.INPUT_AUDIO_SAMPLE_RATE,
        ...
        audioTrack = AudioTrack.Builder()
            ...
            .setAudioFormat(
                AudioFormat.Builder()
                    .setSampleRate(HiveConfig.OUTPUT_AUDIO_SAMPLE_RATE)
            ...
            .setBufferSizeInBytes(
                AudioTrack.getMinBufferSize(
                    HiveConfig.OUTPUT_AUDIO_SAMPLE_RATE,
        ...
        captureThread = Thread({
            ...
            val silenceBuffer = ByteArray(bufferSize) // pre-allocated zeros
            var tapCount = 0
            while (isCapturing) {
                val read = audioRecord?.read(buffer, 0, buffer.size) ?: break
                if (read > 0) {
                    tapCount++
                    // When muted, don't accumulate or send audio
                    if (isMuted) continue
