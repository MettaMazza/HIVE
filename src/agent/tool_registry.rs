...
pub(crate) fn build_default_registries() -> (HashMap<String, ToolTemplate>, HashMap<String, ToolTemplate>) {
    ...
    let mut discord_tools = HashMap::new();
        ...
        let search_timeline = ToolTemplate { name: "search_timeline".into(), system_prompt: "PRIMARY TOOL for recalling past conversations, what users said, and episodic history. Use this FIRST when users ask about conversation history, past interactions, or 'what do you know about me'. Deep search the infinite long-term episodic memory logs. Four modes: 'action:[recent] limit:[N]' — read the N most recent entries (no query needed, default 50). 'action:[search] query:[text] limit:[N]' — word-by-word keyword search (matches ANY word). 'action:[exact] query:[text] limit:[N]' — exact phrase substring search. 'action:[semantic] query:[text] limit:[N]' — VECTOR SIMILARITY search across ALL memory (timeline, synaptic graph, lessons). Use semantic when the user asks broad conceptual questions like 'what do you know about ethics' or 'find conversations about alignment'. You can filter by memory type with source:[timeline], source:[synaptic], or source:[lesson]. SCOPING (CRITICAL): By default, this tool ONLY searches the timeline of the user who sent the current message — it does NOT see your own actions, other users' messages, or documents you created while talking to someone else. In public channels, you MUST use 'scope:[channel]' whenever you need to recall: (1) anything YOU said or created, (2) what happened in this channel generally, (3) documents, PDFs, or artifacts made here, or (4) conversations involving other users. 'scope:[all_public]' searches across ALL public channels. Only omit the scope tag when you specifically want just this one user's history.\n\nCRITICAL DEEP RETRIEVAL GUIDANCE: When retrieving long histories or executing deep queries, DO NOT attempt to pull massive logs all at once with a huge limit, as the engine memory cap will aggressively truncate your output. If the requested information is not fully visible in your current working memory, or your initial tool search indicates truncation, you must paginate. Execute search_timeline with a stable limit (e.g., limit:[50] offset:[0]). To retrieve further back in time, execute the tool again on your next internal cycle with an increased offset (e.g., offset:[50], then offset:[100]). Continue this chunked retrieval incrementally across multiple tool cycles until you have gathered all necessary data to completely fulfill the user's request.".into(), tools: vec![] };
