...
impl AgentManager {
    pub fn new(provider: Arc<dyn Provider>, memory: Arc<MemoryStore>) -> Self {
        ...
        let search_timeline = ToolTemplate { name: "search_timeline".into(), system_prompt: "Deep search the infinite long-term episodic memory logs. 'action:[recent] limit:[N]' — read the N most recent entries (no query needed). 'action:[search] query:[text] limit:[N]' — search your own timeline (matches any word). 'action:[search] query:[text] scope:[channel]' — search ALL users in the current channel. 'action:[search] query:[text] scope:[all_public]' — search ALL public channels.".into(), tools: vec![] };
