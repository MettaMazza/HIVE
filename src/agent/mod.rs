...
impl AgentManager {
    pub fn new(provider: Arc<dyn Provider>, memory: Arc<MemoryStore>) -> Self {
        ...
        let review_reasoning = ToolTemplate { name: "review_reasoning".into(), system_prompt: "Read your historical ReAct reasoning traces from the PERSISTENT timeline (survives beyond the working memory window). Use this to recall why you made decisions from any point in the session, even if those turns have left your ~40 message window. Description format: 'limit:[N]' to retrieve the N most recent reasoning traces (default 5). Also accepts legacy 'turns_ago:[N]' as alias for limit.".into(), tools: vec![] };
